...
// Drawing Bounding Boxes and Writing to text file
...
extern "C" void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output)
{
    try {
        ...
        for (i = 0; i < num; ++i) {
			...
			cout<<names[j]<< " ==="<<names[j];
			int x=int( clock () - begin_time ) /  CLOCKS_PER_SEC;
    ...
    int gaussian_noise, int blur, int num_boxes, int truth_size, float *truth)
{
    ...
    try {
        ...
        if (dsat != 1 || dexp != 1 || dhue != 0) {
            ...
            {
                cv::Mat hsv_src;
                cvtColor(sized, hsv_src, cv::COLOR_RGB2HSV);    
                ...
                cvtColor(hsv_src, sized, cv::COLOR_HSV2RGB);    
            }
            else
            {
                sized *= dexp;
        ...
        if (gaussian_noise) {
            ...
            cv::randn(noise, 0, gaussian_noise); 
...
{
    cv::Mat new_mat(cv::Size(new_img.w, new_img.h), CV_32FC(new_img.c), new_img.data);
